...
export async function performCoinFlip(robot: RobotMotionPort | undefined, wait: (ms: number) => Promise<void> = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), active: () => boolean = () => true): Promise<void> {
  if (!robot || robot.state !== "streaming") return;
  for (const pose of coinFlip()) {
    if (!active() || robot.state !== "streaming") return;
    if (!robot.gotoTarget(toSdkTarget(pose, 0.25))) return;
    await wait(260);
  }
}
