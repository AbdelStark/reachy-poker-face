...
The app sends statement text and a small game state to the configured relay, which sends them to TypeSafe for a Jev judgment. The relay does not intentionally log request bodies or API keys. No statement text is persisted by this app. Numeric settings and, only if the player types a nickname, nickname scores are saved in browser local storage; the UI can clear those scores. The unnamed session score stays in the current tab. A browser speech-recognition provider may process microphone audio off-device when the user presses **Use microphone**; this is independent of the TypeSafe request. The app does not record audio or produce clips.
